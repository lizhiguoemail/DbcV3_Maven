...
<script src="<%=path %>/data/js/diqu/province_street.js"></script>
  <script type="text/javascript">
	$(document).ready(function(){
		var opts12 = {
				data: province_streetdata,
				selStyle: 'margin-left: 3px;',
				select: ['#diqu'],
				//fixWidth: 120,			// 固定宽度
				//head: '---请选择--'	// 自定义
				//root:[15],             // 深层数据入口
				defVal:[${dbc_community.provinceid},${dbc_community.cityid},${dbc_community.districtid},${dbc_community.streetid}], //默认值
				head: "----请选择----"
		};
...
<body style="min-height:500px;">
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <form action="<%=path %>/dbc_community.action?methode=addorupdate" method="post" name="form1">
    <input type="hidden" name="id" value="${dbc_community.id }" />
    <input type="hidden" id="streetid" name="streetid" value="${dbc_community.streetid }" />
    <input type="hidden" id="districtid" name="districtid" value="${dbc_community.districtid }" />
    <input type="hidden" id="cityid" name="cityid" value="${dbc_community.cityid }" />
    <input type="hidden" id="provinceid" name="provinceid" value="${dbc_community.provinceid }" />
    <input type="hidden" name="createdate" value="${dbc_community.createdate }" />
    <input type="hidden" name="createuser" value="${dbc_community.createuser }" />
    <input type="hidden" name="isupdate" value="${isupdate }" />
    <table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        <td width="115px" align="right">社区名称：</td>
        <td>&nbsp;<input name="communityname" type="text" id="communityname" value="${dbc_community.communityname }" /></td>
      </tr>
      <tr>
        <td width="115px" align="right">所在街道：</td>
        <td>&nbsp;
      ...
      <tr>
        ...
        <td>&nbsp;<input name="sortcode" type="text" id="sortcode" value="${dbc_community.sortcode }"  onkeyup="value=value.replace(/[^\d\.]/g,'')"/> 数字越大越靠前</td>
      </tr>
      <tr>
        <td align="right">&nbsp;</td>
