...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_street.action?methode=list';
  ...
  function setdelete(){
	  if(confirm('如删除，该街道下所有的社区都将被一起删除，确认删除选中的街道吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_street.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
	   if(confirm('如删除，该街道下所有的社区都将被一起删除，确认删除选中的街道吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_street.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function showcommunity(svalue){ 
        var f = document.forms[0]; 
        f.action="<%=path %>/dbc_community.action?methode=list";
        var sn=document.getElementById("sname");
        var sv=document.getElementById("svalue");
        var gotopagetype=document.getElementById("gotopagetype");
        sn.options[0].value='streetname';
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="top" background="<%=path %>/images/backstage/content-bg.gif"><span class="autol"><span style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left top transparent;"><b>街道管理</b></span><i style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left bottom transparent;"></i></span></td>
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_street.action?methode=toaddorupdate" class="link3">[新增街道]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="streetname" <c:if test="${sname eq 'streetname'}">selected="selected"</c:if>>街道名称</option>
            <option value="districtname" <c:if test="${sname eq 'districtname'}">selected="selected"</c:if>>所属地区</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="40px">序号</th>
        <th width="200px;">街道名称</th>
        <th width="120px;">所属城区</th>
        <th width="120px;">所属城市</th>
        <th width="120px;">所属省份</th>
        <th width="120px;">社区</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="dbc_street" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${dbc_street.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_street.streetname }</td>
        <td>${districtMap[dbc_street.districtid].districtname }</td>
        <td>${cityMap[dbc_street.cityid].cityname }</td>
        <td>${provinceMap[dbc_street.provinceid].provincename }</td>
        <td><a title="点击查看所有社区" style="text-decoration: underline;color:blue;font-weight: bold;" href="javascript:showcommunity('${dbc_street.streetname}');">查看</a></td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_street.action?methode=toaddorupdate&id=${dbc_street.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${dbc_street.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
