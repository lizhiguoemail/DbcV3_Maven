...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_province.action?methode=list';
  ...
  function setdelete(){
    if(confirm('如删除，该省份下所有的城市、城区、街道、社区都将被一起删除，确认删除选中的省份吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_province.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('如删除，该省份下所有的城市、城区、街道、社区都将被一起删除，确认删除选中的省份吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_province.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_province.action?methode=list";
      f.submit();
    }
    
    function showcity(svalue){ 
        var f = document.forms[0]; 
        f.action="<%=path %>/dbc_city.action?methode=list&islike=0";
        var sn=document.getElementById("sname");
        var sv=document.getElementById("svalue");
        var gotopagetype=document.getElementById("gotopagetype");
        sn.options[0].value='provincename';
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="top" background="<%=path %>/images/backstage/content-bg.gif"><span class="autol"><span style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left top transparent;"><b>省份管理</b></span><i style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left bottom transparent;"></i></span></td>
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_province.action?methode=toaddorupdate" class="link3">[新增省份]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="provincename" <c:if test="${sname eq 'provincename'}">selected="selected"</c:if>>省份名称</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="40px">序号</th>
        <th width="200px;">省份名称</th>
        <th width="120px;">城市</th>
        <th width="60px;">排序码</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="dbc_province" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${dbc_province.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_province.provincename }</td>
        <td>
        	<a title="点击查看所有城市" style="text-decoration: underline;color:blue;font-weight: bold;" href="javascript:showcity('${dbc_province.provincename}');">
        		查看
        	</a>
        </td>
        <td>${dbc_province.sortcode }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_city.action?methode=toaddorupdate&provinceid=${dbc_province.id }" class=link4>添加城市</a>
        &nbsp;<a href="<%=path %>/dbc_province.action?methode=toaddorupdate&id=${dbc_province.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${dbc_province.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
