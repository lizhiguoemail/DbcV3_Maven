...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_article.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_article.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_article.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    }
    
     function setone(id,fd,v){
     if(confirm('确认隐藏选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_article.action?methode=setbyid&id="+id+"&f="+fd+"&v="+v;
       f.submit(); 
	   } 
    }  
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_article.action?methode=list";
      f.submit();
    }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_article.action?methode=toaddorupdate" class="link3">[新增文章]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="title" <c:if test="${sname eq 'title'}">selected="selected"</c:if>>文章标题</option>
            <option value="source" <c:if test="${sname eq 'source'}">selected="selected"</c:if>>文章来源</option>
            <option value="author" <c:if test="${sname eq 'author'}">selected="selected"</c:if>>作者</option>
            <option value="ishidden" <c:if test="${sname eq 'ishidden'}">selected="selected"</c:if>>是否隐藏</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="120px;">文章来源</th>
        <th width="120px;">阅读次数</th>
        <th width="120px;">作者</th>
        <th width="120px;">是否隐藏</th>
        <th width="120px;">排序码</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="article" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${article.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${article.title }</td>
        <td>${treeMap[article.treeid].nodename }</td>
        <td>${article.source }</td>
        <td>${article.readnum }</td>
        <td>${article.author }</td>
        <td>${article.ishidden }</td>
        <td>${article.sortcode }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_article.action?methode=toaddorupdate&id=${article.id }" class=link4>修改</a>
        <c:if test="${article.ishidden eq '隐藏' }">
         &nbsp;<a href="javascript:setone('${article.id }','ishidden','显示');" class=link4>显示</a>
        </c:if>
        <c:if test="${article.ishidden ne '隐藏' }">
         &nbsp;<a href="javascript:setone('${article.id }','ishidden','隐藏');" class=link4>隐藏</a>
        </c:if>
        &nbsp;<a href="javascript:deleteone('${article.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
