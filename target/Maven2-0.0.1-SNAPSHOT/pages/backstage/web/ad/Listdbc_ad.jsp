...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_ad.action?methode=list';
  ...
  function setdelete(){
   ...
   function deleteone(id){
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_ad.action?methode=toaddorupdate" class="link3">[新增站点广告]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="adtype" <c:if test="${sname eq 'adtype'}">selected="selected"</c:if>>广告位置</option>
            <option value="title" <c:if test="${sname eq 'title'}">selected="selected"</c:if>>广告说明</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="40px">序号</th>
        <th width="120px;">广告位置</th>
        <th width="120px;">广告说明</th>
        <th width="120px;">到期时间</th>
        <th width="120px;">添加时间</th>
      <th width="100px">执行操作</th>
      </tr>
      <c:forEach items="${list}" var="dbc_ad" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${dbc_ad.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_ad.adtype }</td>
        <td>${dbc_ad.title }</td>
        <td>${dbc_ad.edate }</td>
        <td>${dbc_ad.createdate }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_ad.action?methode=toaddorupdate&id=${dbc_ad.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${dbc_ad.id }')" class=link4>删除</a>
        &nbsp;获取代码：<input type="text" value="&lt;dbctag:showad adid=&quot;${dbc_ad.id }&quot;&gt;" style="width:180px;"/>
       </td>
      </tr>
    </c:forEach> 
