...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_slides.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_slides.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_slides.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_slides.action?methode=list";
      f.submit();
    }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_slides.action?methode=toaddorupdate" class="link3">[新增幻灯片]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="slides_title" <c:if test="${sname eq 'slides_title'}">selected="selected"</c:if>>标题</option>
            <option value="slides_type" <c:if test="${sname eq 'slides_type'}">selected="selected"</c:if>>分类</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="120px;">显示/隐藏</th>
        <th width="120px;">添加时间</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="dbc_slides" varStatus="v">
      <tr class="hovertr" <c:if test="${dbc_slides.ishidden eq '隐藏'}">style="color:#ccc;"</c:if>>
        <td><input type='checkbox' name="checks"  value="${dbc_slides.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_slides.slides_title }</td>
        <td>${dbc_slides.slides_type }</td>
        <c:if test="${(fn:contains(dbc_slides.slides_img,'http://')==true)||(fn:contains(dbc_slides.slides_img,'https://')==true)}">
        	<td class="bigtext showpic" pic="${dbc_slides.slides_img }">查看</td>
        </c:if>
        <c:if test="${(fn:contains(dbc_slides.slides_img,'http://')==false)&&(fn:contains(dbc_slides.slides_img,'https://')==false)}">
        	<td class="bigtext showpic" pic="<%=basePath %>/${dbc_slides.slides_img }">查看</td>
        </c:if>
        <td>${dbc_slides.ishidden }</td>
        <td>${dbc_slides.createdate }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_slides.action?methode=toaddorupdate&id=${dbc_slides.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${dbc_slides.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
