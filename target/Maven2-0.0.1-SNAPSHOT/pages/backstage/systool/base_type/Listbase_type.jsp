...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_type.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_type.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_type.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_type.action?methode=list";
      f.submit();
    }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="top" background="<%=path %>/images/backstage/content-bg.gif"><span class="autol"><span style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left top transparent;"><b>类型管理</b></span><i style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left bottom transparent;"></i></span></td>
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_type.action?methode=toaddorupdate" class="link3">[新增类型]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="type_name" <c:if test="${sname eq 'type_name'}">selected="selected"</c:if>>类别名称</option>
            <option value="type_value" <c:if test="${sname eq 'type_value'}">selected="selected"</c:if>>类别值</option>
            <option value="type_type" <c:if test="${sname eq 'type_type'}">selected="selected"</c:if>>类别标识</option>
            <option value="type_groupname" <c:if test="${sname eq 'type_groupname'}">selected="selected"</c:if>>类别组名</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="120px;">类别值</th>
        <th width="120px;">类别标识</th>
        <th width="120px;">类别组名</th>
        <th width="120px;">排序码</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="base_type" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${base_type.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${base_type.type_name }</td>
        <td>${base_type.type_value }</td>
        <td>${base_type.type_type }</td>
         <td>${base_type.type_groupname }</td>
         <td>${base_type.sortcode }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_type.action?methode=toaddorupdate&id=${base_type.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${base_type.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
