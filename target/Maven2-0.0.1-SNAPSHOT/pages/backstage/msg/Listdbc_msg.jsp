...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_msg.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_msg.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_msg.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_msg.action?methode=list";
      f.submit();
    }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_msg.action?methode=tosendorshow" class="link3">[发送站内信]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="receiveusername" <c:if test="${sname eq 'receiveusername'}">selected="selected"</c:if>>收件人</option>
            <option value="sendusername" <c:if test="${sname eq 'sendusername'}">selected="selected"</c:if>>发件人</option>
            <option value="title" <c:if test="${sname eq 'title'}">selected="selected"</c:if>>标题</option>
            <option value="content" <c:if test="${sname eq 'content'}">selected="selected"</c:if>>内容</option>
            <option value="state" <c:if test="${sname eq 'state'}">selected="selected"</c:if>>状态</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      ...
      <c:forEach items="${list}" var="dbc_msg" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${dbc_msg.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_msg.receiveusername }</td>
        <td>${dbc_msg.sendusername }</td>
        <td>${dbc_msg.title }</td>
        <td>${dbc_msg.content }</td>
        <td>${dbc_msg.createdate }</td>
        <td><c:if test="${dbc_msg.state eq '已读' }"> <img src="<%=path %>/images/backstage/msg1.gif" alt="已读"/></c:if>
            <c:if test="${dbc_msg.state ne '已读' }"> <img src="<%=path %>/images/backstage/msg0.gif" alt="未读"/></c:if>
        </td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_msg.action?methode=tosendorshow&id=${dbc_msg.id }" class=link4>查看</a>
        &nbsp;<a href="javascript:deleteone('${dbc_msg.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
