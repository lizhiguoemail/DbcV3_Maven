...
<body style="min-height:500px;">
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <form action="<%=path %>/dbc_token.action?methode=addorupdate" method="post" name="form1">
    <input type="hidden" name="id" value="${dbc_token.id }" />
    <input type="hidden" name="createdate" value="${dbc_token.createdate }" />
    <input type="hidden" name="createuser" value="${dbc_token.createuser }" />
    <input type="hidden" name="isupdate" value="${isupdate }" />
    <table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        <td width="115px" align="right">秘钥值：</td>
        <td>&nbsp;<input name="tokenvalue" type="text" id="tokenvalue" value="${dbc_token.tokenvalue }" /></td>
      </tr>
      <tr>
        <td width="115px" align="right">对象id：</td>
        <td>&nbsp;<input name="objid" type="text" id="objid" value="${dbc_token.objid }" /></td>
      </tr>
      <tr>
        <td width="115px" align="right">操作时间：</td>
        <td>&nbsp;<input name="optime" type="text" id="optime" value="${dbc_token.optime }" onkeyup="value=value.replace(/[^\d]/g,'')"/></td>
      </tr>
      <tr>
        <td align="right">排序：</td>
        <td>&nbsp;<input name="sortcode" type="text" id="sortcode" value="${dbc_token.sortcode }"  onkeyup="value=value.replace(/[^\d\.]/g,'')"/> 数字越大越靠前</td>
      </tr>
      <tr>
        <td align="right">&nbsp;</td>
