...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_typegroup.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_typegroup.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_typegroup.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    function ss(){ 
      var f = document.forms[0]; 
      f.action="<%=path %>/dbc_typegroup.action?methode=list";
      f.submit();
    }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="top" background="<%=path %>/images/backstage/content-bg.gif"><span class="autol"><span style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left top transparent;"><b>分类组管理</b></span><i style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left bottom transparent;"></i></span></td>
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_typegroup.action?methode=toaddorupdate" class="link3">[新增类型组]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="name" <c:if test="${sname eq 'name'}">selected="selected"</c:if>>分类组名</option>
            <option value="flag" <c:if test="${sname eq 'flag'}">selected="selected"</c:if>>分类标识</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="40px">序号</th>
        <th width="120px;">分类组名</th>
        <th width="120px;">分类标识</th>
        <th width="120px;">排序码</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="base_typegroup" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${base_typegroup.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${base_typegroup.name }</td>
        <td>${base_typegroup.flag }</td>
        <td>${base_typegroup.sortcode }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_typegroup.action?methode=toaddorupdate&id=${base_typegroup.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${base_typegroup.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
