...
<script src="<%=path %>/data/js/diqu/province_city.js"></script>
  <script type="text/javascript">
	$(document).ready(function(){
		var opts12 = {
				data: province_citydata,
				selStyle: 'margin-left: 3px;',
				select: ['#diqu'],
				//fixWidth: 120,			// 固定宽度
				//head: '---请选择--'	// 自定义
				//root:[15],             // 深层数据入口
				defVal:[${dbc_district.provinceid},${dbc_district.cityid}], //默认值
				head: "----请选择----"
		};
...
<body style="min-height:500px;">
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <form action="<%=path %>/dbc_district.action?methode=addorupdate" method="post" name="form1">
    <input type="hidden" name="id" value="${dbc_district.id }" />
    <input type="hidden" id="cityid" name="cityid" value="${dbc_district.cityid }" />
    <input type="hidden" id="provinceid" name="provinceid" value="${dbc_district.provinceid }" />
    <input type="hidden" name="createdate" value="${dbc_district.createdate }" />
    <input type="hidden" name="createuser" value="${dbc_district.createuser }" />
    <input type="hidden" name="isupdate" value="${isupdate }" />
    <table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        <td width="115px" align="right">地区名称：</td>
        <td>&nbsp;<input name="districtname" type="text" id="districtname" value="${dbc_district.districtname }" /></td>
      </tr>
      <tr>
        <td width="115px" align="right">所在城市：</td>
        <td>&nbsp;
      ...
      <tr>
        ...
        <td>&nbsp;<input name="sortcode" type="text" id="sortcode" value="${dbc_district.sortcode }"  onkeyup="value=value.replace(/[^\d\.]/g,'')"/> 数字越大越靠前</td>
      </tr>
      <tr>
        <td align="right">&nbsp;</td>
