...
<script type="text/javascript" language="javascript">
  ...
  document.forms[0].action='<%=path %>/dbc_community.action?methode=list';
  ...
  function setdelete(){
    if(confirm('确认删除选中的条目吗')){
      var f = document.forms[0];
      f.action="<%=path %>/dbc_community.action?methode=delete";
      f.submit();
    }
  }
   ...
   function deleteone(id){
     if(confirm('确认删除选中的条目吗')){
       var f = document.forms[0]; 
       f.action="<%=path %>/dbc_community.action?methode=deletebyid&id="+id;
       f.submit(); 
		 } 
    } 
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="top" background="<%=path %>/images/backstage/content-bg.gif"><span class="autol"><span style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left top transparent;"><b>社区管理</b></span><i style="background:url('<%=path %>/images/backstage/top_bt.gif') no-repeat scroll left bottom transparent;"></i></span></td>
      ...
      <tr>
        <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_community.action?methode=toaddorupdate" class="link3">[新增社区]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
        <td  align="right">
          <select name="sname" id="sname">
            <option value="communityname" <c:if test="${sname eq 'communityname'}">selected="selected"</c:if>>社区名称</option>
            <option value="streetname" <c:if test="${sname eq 'streetname'}">selected="selected"</c:if>>所属街道</option>
          </select>
    ...
    <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <th width="40px">序号</th>
        <th width="120px;">社区名称</th>
        <th width="120px;">所属街道</th>
        <th width="120px;">所属城区</th>
        <th width="120px;">所属城市</th>
        <th width="120px;">所属省份</th>
      <th width="100px">操作</th>
      </tr>
      <c:forEach items="${list}" var="dbc_community" varStatus="v">
      <tr class="hovertr">
        <td><input type='checkbox' name="checks"  value="${dbc_community.id }" /></td>

        <td>${v.count+(pageParm.nowpage-1)*(pageParm.pagesize)}</td>
        <td>${dbc_community.communityname }</td>
        <td>${streetMap[dbc_community.streetid].streetname }</td>
        <td>${districtMap[dbc_community.districtid].districtname }</td>
        <td>${cityMap[dbc_community.cityid].cityname }</td>
        <td>${provinceMap[dbc_community.provinceid].provincename }</td>
         <td>
        &nbsp;<a href="<%=path %>/dbc_community.action?methode=toaddorupdate&id=${dbc_community.id }" class=link4>修改</a>
        &nbsp;<a href="javascript:deleteone('${dbc_community.id }')" class=link4>删除</a>
       </td>
      </tr>
    </c:forEach> 
