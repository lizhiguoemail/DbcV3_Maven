...
<script type="text/javascript" language="javascript">
	function gotopage(gototype,page)
		...
		document.forms[0].action='<%=path %>/dbc_userinfoadmin.action?methode=listadmin';
		...
		function setdelete(){
		 if(confirm('确认删除选中的条目吗')){
			 var f = document.forms[0];
			 f.action="<%=path %>/dbc_userinfoadmin.action?methode=delete";
			 f.submit();
		 } 
		}
		...
		function deleteone(id){
		 if(confirm('确认删除选中的条目吗')){
			 var f = document.forms[0];
			 f.action="<%=path %>/dbc_userinfoadmin.action?methode=deletebyid&id="+id;
			 f.submit();
		 } 
		}
		...
		function ss(){
			var f = document.forms[0];
			 f.action="<%=path %>/dbc_userinfoadmin.action?methode=list";
			 f.submit();
		}
	</script>
  </head>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
        <tr>
             <td width="20%">&nbsp;<img src="<%=path %>/images/backstage/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;<a href="<%=path %>/dbc_userinfoadmin.action?methode=toaddorupdate&usertype=backstage" class="link3">[新增管理员]</a> <a class="link3" href="javascript:setdelete();">[删除]</a></td>
               <td  align="right">
      ...
      <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
                    ...
                    <c:forEach items="${listuser}" var="user" varStatus="v">
					  <tr class="hovertr">
                        ...
                        <td>${user.qq }</td>
						<td>
						&nbsp;<a href="<%=path %>/dbc_userinfoadmin.action?methode=toaddorupdate&id=${user.id }" class=link4>修改</a>
						&nbsp;<a href="<%=path %>/dbc_userinfoadmin.action?methode=toupdateuser_role&id=${user.id }" class=link4>角色配置</a>
						&nbsp;<a href="<%=path %>/dbc_userinfoadmin.action?methode=toupdateuser_permit&id=${user.id }" class=link4>权限配置</a>
