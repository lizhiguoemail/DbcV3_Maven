 ...
 * 工具类对IndexWriter,IndexReader,IndexSearcher,Analyzer,QueryParser等Lucene这些常用操作对象的获取进行了封装，
 * 其中IndexWriter采用了单例模式，确保始终只有一个对象实例，
 * 因为Lucene限制了索引写操作是阻塞的，即同一时刻只能有一个IndexWriter在执行写操作，直到indexWriter释放lock，
 * 而索引读的操作是可以并发进行的。
 * @author caihuajun
 * @date 2016-01-24
 */
public class Dbc_LuceneManager {
	 private volatile static Dbc_LuceneManager singleton;
	  ...
	  private Dbc_LuceneManager(){}

	  public static Dbc_LuceneManager getInstance() {
	    if (null == singleton) {
	      synchronized (Dbc_LuceneManager.class) {
	        if (null == singleton) {
	          singleton = new Dbc_LuceneManager();
	        }
	      }
	    }
	    return singleton;
